...
internal static class XmlExtentions
{
  ...
  {
    var ns = content.Namespace ?? defaultNs;

    if (ns != null)
    {
      XmlUtils.SplitName(content.Name, out var prefix, out var localName);
  ...
  {
    XmlUtils.SplitName(attr.Name, out var prefix, out var localName);
